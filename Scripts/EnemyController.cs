...
public class EnemyController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (liniar == false)
        {
            changeTime = Random.Range(0.8f, 1.2f);
        }
        else
        {
            changeTime = Random.Range(3.8f, 4.2f);
        }
        timer = changeTime;
        audioSource.Play();
    ...
    public void Fix()
    {
        GameObject destroyEffectObject = Instantiate(destroyEffectPrefab, rigidbody2D.position + Vector2.up * 1.0f, Quaternion.identity);
